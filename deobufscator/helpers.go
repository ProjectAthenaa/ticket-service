...
var (
	...
	stringmodvalmatch = regexp.MustCompile("(\\w{3}) ?%= ?(\\d+);")
	...
	threeletterdef    = regexp.MustCompile("(?:var )([A-Za-z]\\d\\w)=\\+?\"?(\\d{1,4})")
	threeletterarr    = regexp.MustCompile("\\[\\+?([A-Za-z]\\d\\w)]")
	stringfuncresolve = regexp.MustCompile("\\w{3,4}\\.\\w{3}\\((.*?)\\)")
	varindex          = regexp.MustCompile("\\w{3}\\[\\+?\\d{1,4}]")
	varindexset       = regexp.MustCompile("(\\w{3}\\[\\+?\\d{1,4}])=\\+?\"?(\\d{1,3})\"?")
	...
	cparamreturn = regexp.MustCompile("return (\\w{3}\\[\\d+])")
	...
	for returns > 0 {
		...
		case "binary":
			...
			if conditiontype[0] == 0 && conditiontype[1] == 0 {
				retstring += cases[nextcase].Body + fmt.Sprintf("{%s}{%s}", switchwalker(cases[nextcase].Routeone, escapes, cases), switchwalker(cases[nextcase].Routetwo, escapes, cases))
				returns--
			} else if conditiontype[0] > 0 && conditiontype[1] == 0 {
				escapes = append(escapes, nextcase)
				retstring += cases[nextcase].Body + fmt.Sprintf("{%s}", switchwalker(cases[nextcase].Routeone, escapes, cases))
				nextcase = cases[nextcase].Routetwo
			} else if conditiontype[0] == 0 && conditiontype[1] > 0 {
				escapes = append(escapes, nextcase)
				retstring += cases[nextcase].Body + fmt.Sprintf("{%s}", switchwalker(cases[nextcase].Routetwo, escapes, cases))
...
func o5f(b4g, s4g string, initval, addval, modval int) string {
	var C6f strings.Builder
	F4g, _ := url.PathUnescape(b4g)
	i4g := initval
	for w4g := 0; w4g < len(F4g); w4g++ {
		var M4g = charCodeAt(F4g, w4g)
		var r4g = M4g % 256
		var W4g = (M4g - r4g) / 256
		var Q4g = charCodeAt(s4g, w4g%len(s4g))
		var J4g = r4g ^ i4g ^ Q4g
		i4g += addval
		i4g %= modval
		C6f.WriteString(fromCharCode(W4g*256 + J4g))
	}
	return C6f.String()
}
...
func helper(rawfuncstr, decodestr string) string {
	modval := stringmodvalmatch.FindStringSubmatch(rawfuncstr)
	addregex := regexp.MustCompile(fmt.Sprintf("%s ?\\+= ?(\\d+);", modval[1]))
	initregex := regexp.MustCompile(fmt.Sprintf("%s ?= ?(\\d+);", modval[1]))
	addval := addregex.FindStringSubmatch(rawfuncstr)
	initval := initregex.FindStringSubmatch(rawfuncstr)
	modint, _ := strconv.Atoi(modval[2])
	addint, _ := strconv.Atoi(addval[1])
	initint, _ := strconv.Atoi(initval[1])
	hashint := strconv.Itoa(c5f(N5f(rawfuncstr)))
	return o5f(decodestr, hashint, initint, addint, modint)
}
func decodeStrings(scriptin string) []string {
	var retarr []string
	obfuscatedstrings := obffuncregex.FindAllStringSubmatch(scriptin, -1)
	for _, strarr := range obfuscatedstrings {
		result := helper(strarr[1], strarr[2])
...
func mainstringarrresolve(scriptin string) []string {
	...
	modval := stringmodvalmatch.FindStringSubmatch(scriptin)
	addregex := regexp.MustCompile(fmt.Sprintf("%s ?\\+= ?(\\d+);", modval[1]))
	initregex := regexp.MustCompile(fmt.Sprintf("%s ?= ?(\\d+);", modval[1]))
	addval := addregex.FindStringSubmatch(scriptin)
	initval := initregex.FindStringSubmatch(scriptin)
	modint, _ := strconv.Atoi(modval[2])
	addint, _ := strconv.Atoi(addval[1])
	initint, _ := strconv.Atoi(initval[1])
	outstr := o5f(returnR.FindStringSubmatch(scriptin)[1], e9j, initint, addint, modint)
	outstr = strings.ReplaceAll(outstr, "\"", "'")
	arrout := strings.Split(outstr, splitR.FindStringSubmatch(scriptin)[1])
...
func cparamresolver(scriptin string, varmap map[string]string, boolarr map[int]map[int]int, stringarrin []string) string {
	roundvarmap := map[string]string{}
	for _, val := range threeletterdef.FindAllStringSubmatch(scriptin, -1) {
		roundvarmap[val[1]] = val[2]
	}
	for _, val := range threeletterarr.FindAllStringSubmatch(scriptin, -1) {
		scriptin = strings.Replace(scriptin, val[0], "["+roundvarmap[val[1]]+"]", 1)
	}
	scriptin = switchFlattener(replaceBoolArrays(globalvarreplacer(scriptin, varmap), boolarr))
	for _, val := range varindexset.FindAllStringSubmatch(scriptin, -1) {
		roundvarmap[val[1]] = val[2]
	}
	for _, val := range stringfuncresolve.FindAllStringSubmatch(scriptin, -1) {
		indexval := varindex.FindStringSubmatch(val[1])
		if len(indexval) > 0 {
			scriptin = strings.Replace(scriptin, indexval[0], roundvarmap[indexval[0]], -1)
		}
	}
	for _, val := range stringfuncresolve.FindAllStringSubmatch(scriptin, -1) {
		var stringindex int
		if len(digitmatch.FindStringSubmatch(val[1])) == 0 {
			stringindex = 0
		} else {
			stringindex, _ = strconv.Atoi(digitmatch.FindStringSubmatch(val[1])[0])
		}
		scriptin = strings.ReplaceAll(scriptin, val[0], "\""+stringarrin[stringindex]+"\"")
	}
	keys := []string{}
	bodies := []string{}
	nextbegin := strings.Index(scriptin, "{")
	...
	funcpush := bodies[len(bodies)-5]
	valtomatch := cparamreturn.FindStringSubmatch(funcpush)
	var outstr strings.Builder
	for _, val := range regexp.MustCompile(fmt.Sprintf("%s\\+?=?\"(\\w+)\"", strings.Replace(valtomatch[1], "[", "\\[", 1))).FindAllStringSubmatch(funcpush, -1) {
		outstr.WriteString(val[1])
	}
...
func Cparamarray() []int {
	token := make([]byte, 8)
	if _, err := rand.Read(token); err != nil {
		fmt.Println(err)
	}
	token = append(token, 8, 8, 8, 8, 8, 8, 8, 8)
	var retToken []int
	for _, b := range token {
		retToken = append(retToken, int(b))
	}
	return retToken
}
